...
内存使用: ' . memory_get_usage(true) . '
峰值内存: ' . memory_get_peak_usage(true) . '</code></pre>
            </div>';
    } else {
        $output .= '<div class="error-section">
                <h3>提示:</h3>
    ...
    $output .= '<div class="error-footer">
                <hr>
                <p>XQPF V:' . (defined('FRAMEWORK_VERSION') ? FRAMEWORK_VERSION : '1.0') . ' | ' . date('Y-m-d H:i:s') . '</p>
            </div>
        </div>';
